...
public class YandexPoints {
	...
	public void startRequest(String query, int results)
	{
	    ...
	    this.url = "http://psearch-maps.yandex.ru/1.x/" +
	    			"?text=" + query + "&format=json&results=" + results +
	...
	private class DownloadWebPageTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... urls) {
	        try {
	            ...
	            // Get cache dir (try to use external memory, if n/a, use internal)	            
		...
		// Get cache file (try to get from external storage)
			...
			try {
		        ...
		        JSONObject GeoObjectCollection = object.getJSONObject("response").getJSONObject("GeoObjectCollection");
		        JSONArray elementsArray = GeoObjectCollection.getJSONArray("featureMember"); 
		        ...
		        for(int i = 0; i < len; i++)
		        {
		        	...
		        	Point.name = temp.getJSONObject("GeoObject").getString("name");
		        	coordinates = temp.getJSONObject("GeoObject").getJSONObject("Point").getString("pos");
